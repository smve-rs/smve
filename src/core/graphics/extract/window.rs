...
fn e_extract_windows(
    ...
    for (entity, window, handle, primary) in main_world_query.iter() {
        ...
        let extracted_window = extracted_windows.windows.entry(entity).or_insert(ExtractedWindow {
        ...
        extracted_window.size_changed = new_width != extracted_window.physical_width || new_height != extracted_window.physical_height;
