...
fn extract(world: &mut World, app: &mut App) {
    ...
    let scratch_world = world.remove_resource::<ScratchMainWorld>().expect("ScratchMainWorld should exist");
    ...
    let inserted_world = app.world.remove_resource::<MainWorld>().expect("MainWorld should exist");
